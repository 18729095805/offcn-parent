...
public class UserLoginController {
    ...
    public AppResponse<Object> sendCode(String phoneNo) {
        ...
        stringRedisTemplate.opsForValue().set(phoneNo, code, 60 * 60 * 24 * 7, TimeUnit.SECONDS);//键 值 保存时间 秒
    ...
    public AppResponse<UserRespVo> login(String username, String password) {
        ...
        if (member == null) { //登录不成功
            AppResponse<UserRespVo> fail = AppResponse.fail(null); //静态方法 类名.方法名,可以直接使用方法,不用创建对象.
        ...
        //发送令牌
        //生成令牌 唯一且不包含"-"
        String token = UUID.randomUUID().toString().replace("-", ""); //replace方法替换字符串
        UserRespVo userRespVo = new UserRespVo();
        userRespVo.setAccessToken(token);
        BeanUtils.copyProperties(member, userRespVo);  //拷贝属性
        //令牌存入redis  令牌作为键,用户id作为值
        // 如果反过来用户id作为键,没有登录成功时,不能获取到用户id
    ...
    @ApiImplicitParams(value = {
            @ApiImplicitParam(value = "用户名id", name = "id", required = true)
    ...
    public AppResponse<UserRespVo> findUser(@PathVariable("id") Integer id) {
        ...
        UserRespVo userRespVo = new UserRespVo();
        BeanUtils.copyProperties(member, userRespVo); //将前面的属性拷贝给后面的方法
        return AppResponse.ok(userRespVo);
    }
}
